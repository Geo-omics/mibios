...
from mibios.query import FKCache, QuerySet
...
class Table(Table0):
    ...
    def auto_tweak_queryset(self, columns):
        # TODO: yet unused, what was this for?
        if not type(self).__name__.endswith('AutogeneratedTable'):
            return

        qs = self.data.data
        for col in columns:
            field = col.accessor.get_field(self._meta.model)
            if field is not None and field.many_to_one:
                # FK
    ...
    def as_values(self, exclude_columns=None):
        ...
        if isinstance(self.data.data, QuerySet):
            ...
            cache_fields = []
            onlies = []
            force_only = True  # TODO: ignore this for now
            for i in columns:
                if field := i.accessor.get_field(self._meta.model):
                    if field.many_to_one:
                        if qs.query.select_related is True:
                            # don't use cache if select_related() was called
                            # without args (as all get selected)
                            pass
                        elif qs.query.select_related is False:
                            cache_fields.append(field)
                        else:
                            # select_related can be iterated
                            if field.name not in qs.query.select_related:
                                cache_fields.append(field)
            ...
            if cache_fields:
                self.rel_values_cache = FKCache(self._meta.model, cache_fields)
            else:
                self.rel_values_cache = {}

            # TODO loading_fields, defer = qs.query.deferred_loading
            # test if defer() or only() were called previously
            # TODO previous_defer_only = bool(loading_fields) or not defer
            # TODO if force_only or previous_defer_only:
            print(f'BORK ONLY {onlies=} {cache_fields=}')
            qs = qs.add_only(*onlies)
            self.data.data = qs.iterator2(cache=self.rel_values_cache)
    ...
    def as_values_bottom(self, columns):
        ...
        It may be slow for large datasets so can be overwritten by inheriting
        classes to be speedier.
        """
        for row in self.rows:
            yield [force_str(row.get_cell_value(col.name)) for col in columns]


def table_factory(model, table=Table):
    attrs = {}
    for i in model._meta.get_fields():
        break
        if i.many_to_one:
            print(f'BORK FACTORY {i=}')

            def value_FOO(self, record):
                breakpoint()
                return self.rel_values_cache[i.name][record.attname]
...
class ReadAbundanceTable(Table):
    ...
    class Meta:
        ...
        # order_by = ['-target_cov']  can't order here, to expensive?
        exclude = ['id']
