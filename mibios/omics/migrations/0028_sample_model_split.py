# Generated by Django 4.2.23 on 2025-09-18 17:25

from django.conf import settings
import django.contrib.postgres.fields
import django.contrib.postgres.indexes
from django.db import migrations, models
import django.db.models.deletion
import mibios.fields


# field common between old Sample and new SeqSample, must be copied during
# migration
fields = ['sample_id', 'sample_name', 'sample_type', 'sra_accession',
          'gold_analysis_id', 'gold_seq_id', 'amplicon_target', 'fwd_primer',
          'rev_primer', 'analysis_dir', 'read_count', 'reads_mapped_contigs',
          'reads_mapped_genes', 'access', 'notes']


def populate_seqsample(apps, schema_editor):
    Sample = apps.get_model('glamr', 'Sample')
    SeqSample = apps.get_model('omics', 'SeqSample')

    for i in Sample.objects.all():
        kwargs = {k: getattr(i, k) for k in fields}
        obj = SeqSample(pk=i.pk, parent=i, **kwargs)
        obj.full_clean()
        obj.save()


def refill_removed_sample_fields(apps, schema_editor):
    """
    Refill lost fields in Sample model.  Assumes there is still an 1-1 relation
    between the models
    """
    Sample = apps.get_model('glamr', 'Sample')
    SeqSample = apps.get_model('omics', 'SeqSample')

    for i in SeqSample.objects.all():
        obj = Sample.objects.get(sample_id=i.sample_id)
        for k in fields:
            if k == 'sample_id':
                continue
            setattr(obj, k, getattr(i, k))

        obj.full_clean()
        obj.save()


class Migration(migrations.Migration):

    dependencies = [
        migrations.swappable_dependency(settings.OMICS_SAMPLE_MODEL),
        ("omics", "0027_alter_bin_options_alter_bin_managers_and_more"),
        ("glamr", "0028_remove_sample_contig_abundance_loaded_and_more"),
    ]

    operations = [
        migrations.CreateModel(
            name="SeqSample",
            fields=[
                ("id", mibios.fields.AutoField(primary_key=True, serialize=False)),
                (
                    "sample_id",
                    models.CharField(
                        help_text="internal sample accession",
                        max_length=32,
                        unique=True,
                    ),
                ),
                (
                    "sample_name",
                    models.TextField(
                        blank=True,
                        default="",
                        help_text="sample ID or name as given by original data source",
                        max_length=32,
                    ),
                ),
                (
                    "sample_type",
                    models.CharField(
                        blank=True,
                        choices=[
                            ("amplicon", "amplicon"),
                            ("metagenome", "metagenome"),
                            ("metatranscriptome", "metatranscriptome"),
                            ("isolate_genome", "isolate_genome"),
                        ],
                        default=None,
                        max_length=32,
                        null=True,
                    ),
                ),
                (
                    "sra_accession",
                    models.TextField(blank=True, default="", max_length=16),
                ),
                (
                    "gold_analysis_id",
                    models.TextField(blank=True, default="", max_length=32),
                ),
                (
                    "gold_seq_id",
                    models.TextField(blank=True, default="", max_length=32),
                ),
                (
                    "amplicon_target",
                    models.TextField(blank=True, default="", max_length=16),
                ),
                ("fwd_primer", models.TextField(blank=True, default="", max_length=32)),
                ("rev_primer", models.TextField(blank=True, default="", max_length=32)),
                (
                    "analysis_dir",
                    models.TextField(
                        blank=True,
                        default=None,
                        help_text="path to results of analysis, relative to OMICS_PIPELINE_DATA",
                        null=True,
                    ),
                ),
                (
                    "read_count",
                    models.PositiveIntegerField(
                        blank=True,
                        default=None,
                        help_text="number of read pairs (post-QC) used for assembly mapping",
                        null=True,
                    ),
                ),
                (
                    "reads_mapped_contigs",
                    models.PositiveIntegerField(
                        blank=True,
                        default=None,
                        help_text="number of reads mapped to contigs",
                        null=True,
                    ),
                ),
                (
                    "reads_mapped_genes",
                    models.PositiveIntegerField(
                        blank=True,
                        default=None,
                        help_text="number of reads mapped to genes",
                        null=True,
                    ),
                ),
                (
                    "parent",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        to=settings.OMICS_SAMPLE_MODEL,
                    ),
                ),
                (
                    "access",
                    django.contrib.postgres.fields.ArrayField(
                        base_field=models.SmallIntegerField(
                            blank=True, default=None, null=True
                        ),
                        blank=True,
                        default=None,
                        null=True,
                        size=None,
                    ),
                ),
                ("notes", models.TextField(blank=True, default="")),
            ],
        ),
        migrations.RunPython(populate_seqsample, refill_removed_sample_fields),
        migrations.DeleteModel(
            name="Dataset",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="amplicon_target",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="analysis_dir",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="dataset",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="fwd_primer",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="has_paired_data",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="read_count",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="reads_mapped_contigs",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="reads_mapped_genes",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="rev_primer",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="sample_id",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="sample_name",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="sample_type",
        ),
        migrations.RemoveField(
            model_name="sample",
            name="sra_accession",
        ),
        migrations.AlterField(
            model_name="gene",
            name="sample",
            field=models.ForeignKey(
                on_delete=django.db.models.deletion.CASCADE, to="omics.seqsample"
            ),
        ),
        migrations.AlterField(
            model_name="ncrna",
            name="sample",
            field=models.ForeignKey(
                on_delete=django.db.models.deletion.CASCADE, to="omics.seqsample"
            ),
        ),
        migrations.AlterField(
            model_name="readlibrary",
            name="sample",
            field=models.OneToOneField(
                on_delete=django.db.models.deletion.CASCADE,
                related_name="reads",
                to="omics.seqsample",
            ),
        ),
        migrations.AddIndex(
            model_name="seqsample",
            index=django.contrib.postgres.indexes.GinIndex(
                fields=["access"], name="seqsample_access_gin", opclasses=["array_ops"]
            ),
        ),
    ]
